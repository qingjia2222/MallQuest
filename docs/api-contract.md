...
身份入口严格分离：游客/会员使用 `wx-login → scan → visitor workspace`；商户使用 `store-code → merchant workspace`；管理者使用 `web-login → manager workspace`。前端不在登录后提供角色切换，后端同时按 token channel 与访问关系表阻断跨角色请求。模型和用户工具参数不能覆盖后端会话中的 `user_id/mall_id/session_id`。
